...
## 9. Mie grid·truncation 정책 (2026-08-21 확정)

- exact FR631: raw phase, broad truncation OFF가 canonical scientific reference이다.
- OFF의 이유는 broad 영향이 작아서가 아니다. `0–0.005°` local cap은 무시 가능한 영향이었지만, OSOAA-style broad truncation은 directional `Rrs/rrs`를 수 % 변화시키고 일부 조건에서 입자산란의 큰 비율을 transformed term으로 이동시킨다.
- recognized legacy 361×0.5°: forward under-resolution 가능성을 경고하고 FR631 교체를 우선 권고한다. `--ocrt-mie-truncation`은 `OCRT_ADVANCED=1`이 필요한 matched-reference fallback이며 자동 활성화하지 않는다.
- unknown non-FR631 custom grid: 비-FR631이라는 이유만으로 저해상도라고 단정하지 않는다. forward-angle resolution과 RT convergence 확인 전까지 unvalidated이다.
- broad 비교는 OCRT와 reference에서 동일한 transform, `A`, `b/omega/tau`, vector moments, source correction 및 physical depth를 사용해야 한다.

상세: `01_OCRT_C/docs/OCRT_MIE_GRID_TRUNCATION_AND_VALIDATION_ARTIFACT_POLICY_2026-08-21.md`

## 10. 산포도·CSV package 동기화 정책

모든 정량 검증 그림은 그림 생성에 사용한 authoritative CSV와 함께 `05_VALIDATION/artifacts/<campaign_id>/`에 저장한다.

```text
05_VALIDATION/artifacts/<campaign_id>/
  CAMPAIGN_METADATA.json
  PLOT_DATA_MAP.csv
  authoritative/paired CSV
  derived metrics CSV
  scatter/residual PNG
  repro/ 또는 plotting scripts
  SHA256SUMS.txt
  ARTIFACT_MANIFEST.json
```

자료를 재처리하면 source CSV, metrics, 모든 관련 figure, map, metadata와 SHA manifest를 같은 revision으로 재생성하고 campaign directory를 atomic replacement한다. 그림만 또는 CSV만 부분 교체하는 것은 금지한다.
...
기술문서 전체를 코드에 복사하지 않는다. 구현 위치의 주석에 물리·수치 결정, 기본값, 적용범위, reference matching 조건, 부작용과 Gate를 요약하고 `DOC-REF`로 상세 문서를 연결한다. 코드·주석·문서·시험·CSV·그림은 같은 변경단위에서 갱신한다.
