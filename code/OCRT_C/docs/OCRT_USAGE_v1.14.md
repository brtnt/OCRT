...
- 2026-07-15 옵션 정리 5차(G4 해면·계면 확정): `--surface` **필수화**,
  `--wind-speed` ocean/coxmunk **필수화**(0 명시 허용), `--n-water` 기본
...
- `--wind-speed U` [m/s] : Cox-Munk 거칠기 (0=flat Fresnel).
  **ocean/coxmunk 에서 필수다(2026-07-15; 0 도 명시, 배치는 행 단위 검사).**
...
- `--surface` 는 **필수**다(black|flat|coxmunk|ocean; 조용한 black 기본 폐지,
  ccrr-compare 모드만 예외). lambert 는 미구현 명시 차단.
- `--q-convention`(0=legacy (Rs−Rp)/2, 1=Hansen/Mishchenko=기본)과
  `--sigma-type`(0=Nakajima-Tanaka, 1=Cox-Munk 1954=기본)은 **ADVANCED
  게이트**다(기본 1 재명시는 통과).
...
OMP_NUM_THREADS=1 ./build/v2_solver_vk --surface coxmunk --wind-speed 3 --sza 40 \
