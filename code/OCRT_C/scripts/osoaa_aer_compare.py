...
os.makedirs("/tmp/oaer",exist_ok=True); os.makedirs("/tmp/mie_dir",exist_ok=True)

def run(band,sza,aot865):
    aot=aot865*RATIO[band]; res=f"/tmp/oaer/{band}_{sza}_{aot865}"
    ...
    cmd=[EXE,"-OSOAA.ResRoot",res,"-OSOAA.Log","M.Log","-OSOAA.Wa",WUM[band],
         ...
         "-AP.MOT","0.0","-AP.HR","8.0","-AP.HA","2.0",
...
# append 저장
out="cmp_item1_aerosol.csv"; new=not os.path.exists(out)
