#!/usr/bin/env python3
"""item#2 pure ocean: OCRT ↔ OSOAA, TOA IQU + Rrs(0-) 동시(한 OSOAA run 북킵핑). raa=90.
- OCRT: --surface ocean --water-model ocrt --ocrt-chl 0 --ocrt-tsm 0 --ocrt-adom440 0 --pressure 1013.25, 한 run에 TOA rho_I/Q/U + rrs0minus.
- OSOAA: pure-water(Chl0,YS0,AP.MOT), 한 run에 vsVZA REFL(TOA I) + Adv level0(TOA Q/U) + Adv level27(0-) + Flux Ed.
- TOA: air-side VZA(#1과 동일). Rrs(0-): water-side Snell (OCRT air-vza θ → water-vza arcsin(sinθ/1.34)).
- 규약: REFL/glint-free/U부호(OCRT_U=-OSOAA_U)/PCHIP/nadir Q는 |Q|.
"""
...
def parse_osoaa(res,sza):
    ...
    # Adv_UP level0(TOA Q/U) + level27(0- I/Q/U), VZA水side at 27
    def adv(level):
        V,I,Q,U=[],[],[],[]
    ...
    # Ed at level 0 and 27 (Flux col5)
    ed={}
...
def run_ocrt(band,sza,vza):
    out=subprocess.run([OCRT,"--surface","ocean","--decouple-sunglint","--gas-column-h2o","0","--gas-column-o3","0","--gas-column-no2","0","--gas-column-o2","0","--gas-column-co2","0","--gas-column-ch4","0","--water-model","ocrt","--ocrt-chl","0","--ocrt-tsm","0","--ocrt-adom440","0","--wind-speed","3",
    ...
    # 첫 3토큰 = TOA rho_I/Q/U; rrs0minus= 라벨
    toks=out.split()
    ...
    return rI,rQ,rU,d.get('rrs0minus',np.nan)

sza_filter=int(sys.argv[1]) if len(sys.argv)>1 else None
rows=[]
for band in BANDS:
    for sza in SZA:
        if sza_filter is not None and sza!=sza_filter: continue
        res=osoaa(band,sza)
        if not res: sys.stderr.write(f"OSOAA FAIL {band} {sza}\n"); continue
        P=parse_osoaa(res,sza); mu=P['mu']
        for va in VZA_AIR:
            cI,cQ,cU,crrs=run_ocrt(band,sza,va)
            # OSOAA TOA: REFL(air va) for I, Adv level0 (air va) Q/U /mu
            oI=float(P['fREFL'](va)); oQ=float(P['fQ0'](va))/mu; oU=float(P['fU0'](va))/mu
            # OSOAA Rrs(0-): water-side vza, I(27)/Ed27
            vw=water_vza(va); oRrs=float(P['fI27'](vw))/P['Ed27']
            rows.append((band,sza,va,vw, cI,oI,cQ,oQ,cU,oU, crrs,oRrs))
out="cmp_item2_pureocean.csv"; new=not os.path.exists(out)
with open(out,"a",newline="") as f:
    w=csv.writer(f)
    if new: w.writerow(["band","sza","vza_air","vza_water","OCRT_TOA_I","OSOAA_TOA_I","OCRT_TOA_Q","OSOAA_TOA_Q","OCRT_TOA_U","OSOAA_TOA_U","OCRT_rrs0minus","OSOAA_rrs0minus"])
    w.writerows(rows)
import statistics
dI=[100*(r[4]/r[5]-1) for r in rows if r[5]]
dR=[100*(r[10]/r[11]-1) for r in rows if r[11] and r[2]==0]  # Rrs nadir
if dI: print(f"sza={sza_filter}: TOA_I MAPE={statistics.mean(abs(x) for x in dI):.2f}% | Rrs(0-)@nadir bias={statistics.mean(dR):+.2f}% (버그)" if dR else f"TOA_I MAPE={statistics.mean(abs(x) for x in dI):.2f}%")
