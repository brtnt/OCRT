...
이 문서는 개발 상태의 **누적 기록**이다(§1–§14, 작성일 순). 새 작업 세션의 시작 절차와 자료 위치는
`MIGRATION_README_2026-09-05_KO.md`(같은 디렉터리)에 있고, 사용자용 실행 절차는 `docs/manual/` 이 담당한다.
위치: `docs/status/` (2026-09-14 저장소 루트에서 이동). §1의 폴더 구조는 2026-08-23 당시 로컬 폴더 기준이며,
그 뒤의 변경(git 공개 범위, 제3자 코드 제외, 문서 재배치)은 MIGRATION README §3 을 따른다.
