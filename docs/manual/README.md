# OCRT 사용자 매뉴얼 / User manual

**수정 원본은 LaTeX이다. / LaTeX is the authoritative source.**

| 문서 / Document | 파일 / File |
|---|---|
| 한글판 PDF | [OCRT_Manual_KO.pdf](OCRT_Manual_KO.pdf) |
| English PDF | [OCRT_Manual_EN.pdf](OCRT_Manual_EN.pdf) |
| LaTeX 원본과 빌드 안내 / Sources and build instructions | [latex/README.md](latex/README.md) |
| 한글판 진입 파일 / Korean entry point | [latex/ocrt_manual_ko.tex](latex/ocrt_manual_ko.tex) |
| 영문판 진입 파일 / English entry point | [latex/ocrt_manual_en.tex](latex/ocrt_manual_en.tex) |
| 실행 가능한 예시 / Executable examples | [examples/](examples/) |
...
On Linux, use `python3` if `python` is unavailable. See the build instructions for XeLaTeX, fonts and Windows/WSL support. Both PDFs are written to this directory. Keep option names, units and defaults consistent between editions, and review the rendered pages after updates.

## 구성 / Contents

매뉴얼 앞부분의 **Quick start / 빠른 시작**은 최소 준비, 단일 계산, 12행 Rayleigh LUT 생성과 정상 실행 확인을 안내한다. Linux/WSL과 Windows PowerShell 명령을 함께 제공한다.
...
1. 기본 개념과 관측 기하 / Concepts and viewing geometry, including the RAA convention
2. 설치와 첫 실행 / Installation and first run
3. C 실행 프로그램 전체 옵션 / Complete C option reference
4. 입력 파일과 배치 CSV 계약 / Input files and batch CSV contracts
5. 출력 형식과 결과 해석 / Output formats and interpretation
6. 검증·문제 해결·문서 근거 / Verification, troubleshooting and provenance
...
| 부록 / Appendix | 내용 / Contents |
|---|---|
| A | 상세 실행 예시와 LUT 제작 / Worked examples and LUT generation |
| B | Python 사용과 배치 자료 생성 / Python and batch data generation |
| C | 핵심 사용 예시 10개 / 10 core worked examples |
| D | C LUT 예시 도구 / C LUT example utility |
| E | 환경변수 소스 색인 / Environment-variable source index |
...
**OCRT RAA=180°는 선글린트 가지다. / OCRT RAA=180° is the direct-sunglint branch.** 각도 변환과 Stokes U의 부호는 1장을 읽고 확인한다.
