...
`technical/` and `validation/` record the state at the time they were written. For the current usage, consult the manual together with the source of the corresponding version. Do not assume that the options and defaults in older documents match the current code.
...
`technical/`, `validation/`은 작성 당시 상태를 기록한 자료다. 현재 실행법은 매뉴얼과 해당 버전 소스를 함께 확인한다. 과거 문서의 옵션·기본값이 현재 코드와 같다고 가정하지 않는다.
